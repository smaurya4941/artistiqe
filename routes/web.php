...
use App\Http\Controllers\FreeRegistrationController;
use App\Http\Controllers\ArtistRegisterController;
use App\Http\Controllers\CollectorRegisterController;
use App\Http\Controllers\GalleryRegisterController;
...
Route::get('/gallery/register', [GalleryRegisterController::class, 'create'])
    ->name('gallery.register');

Route::post('/gallery/register', [GalleryRegisterController::class, 'store'])
    ->name('gallery.register.store');

Route::get('/gallery-register-success', function () {
    return view('auth.boxed.gallery-success');
})->name('gallery.register.success');


Route::get('/collector/register', [CollectorRegisterController::class,'create'])
    ->name('collector.register');

// 2️⃣ Form submit (POST)
Route::post('/collector/register', [CollectorRegisterController::class,'store'])
    ->name('collector.register.store');

// 3️⃣ Success page
Route::get('/collector-register-success', function () {
    return view('auth.boxed.collector-success');
})->name('collector.register.success');


Route::post('/artist-register', [ArtistRegisterController::class,'store'])
    ->name('artist.register.store');

Route::get('/artist-register-success', function () {
    return view('auth.boxed.artist-success');
})->name('artist.register.success');
...
// Registration Page
Route::get('/free-registration', function () {
    return view('frontend.free-registration');
});

Route::post('/free-registration', [FreeRegistrationController::class, 'store'])->name('free.registration.store');

Route::get('/check', function () {
    return "Route working fine!";
});
...
// routes/web.php
Route::get('/skip-verification', function () {
    if (!auth()->check()) {
    ...
    // Decide target based on your app’s roles
    if ($user->user_type === 'seller') {
        $target = route('seller.dashboard');   // make sure THIS route does not use 'verified'
    } elseif ($user->user_type === 'admin') {
        $target = route('admin.dashboard');    // same here
    } else {
        $target = route('dashboard');          // and here
    }

    // Redirect away so browser history doesn’t come back to verify page
    return redirect()->away($target);
})->middleware(['web','auth'])->name('skip.verification');
