                        ...
                        <div class="row no-gutters">
                            ...
                            <div class="col-lg-6 p-4 p-lg-5 d-flex flex-column justify-content-center border right-content" style="height: auto;">
                                ...
                                <div class="text-center text-lg-left">
                                    <h1 class="fs-20 fs-md-24 fw-700 text-primary no-dot" style="text-transform: uppercase;">{{ translate('CREATE AN ACCOUNT')}}</h1>
                                </div>-->
                                
                                <!-- Titles -->
@php
  // Get the translated string safely
  $rawTitle = translate('Create an account');

  // 1) Trim whitespace
  $rawTitle = trim(strip_tags($rawTitle));

  // 2) Remove any trailing dot or danda
  $noTrail = preg_replace('/[.।]+\\s*$/u', '', $rawTitle);

  // 3) Extra safety: remove any dot/danda anywhere (in case the theme injects punctuation in the string itself)
  $cleanTitle = str_replace(['.', '।'], '', $noTrail);
@endphp

<div class="text-center text-lg-left">
  <h1 class="fs-20 fs-md-24 fw-700 text-primary text-uppercase no-after">
    <span class="no-after-text">{{ mb_strtoupper($cleanTitle) }}</span>
  </h1>
</div>

                                
    <style>
  /* Kill ANY pseudo-element that might inject a dot */
  .no-after::after,
  .no-after::before,
  .no-after-text::after,
  .no-after-text::before,
  .text-center.text-lg-left::after,
  .text-center.text-lg-left::before,
  h1.no-after::after,
  h1.no-after::before,
  .text-primary.no-after::after,
  .text-primary.no-after::before {
