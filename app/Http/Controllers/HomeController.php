...
class HomeController extends Controller
{
    ...
    {
          $homeBlogs = Blog::where('status', 1)
        ->latest()
        ->take(2)
        ->get();

    // Featured slider blogs (NEW – required)
    $featuredBlogs = Blog::where('status', 1)
    ->latest()
    ->take(5)
    ->get()
    ->map(function ($blog) {
        $blog->banner_url = uploaded_asset($blog->banner);
        return $blog;
    });


    return view('frontend.metro.index', compact('homeBlogs', 'featuredBlogs'));
    ...
    public function shop($slug)
{
    ...
    // Redirect /shop/{slug} to /shop/{slug}/all-products
    return redirect()->route('shop.visit.type', ['slug' => $slug, 'type' => 'all-products']);
}
